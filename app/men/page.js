...
// Import some images for products
...
export default function MenCategoryPage() {
    ...
    const handleOfferClick = (idx) => {
    const updatedOffers = specialOffers.map((offer, i) => ({
        ...offer,
        active: i === idx // শুধু ক্লিক করা ইনডেক্সটি true হবে, বাকি সব false
    }));
    setSpecialOffers(updatedOffers);
    router.push('/men'); // পেজ রিডাইরেক্ট (যদি দরকার হয়)
};
    ...
    const sidebarSubcategories = [
        { name: 'Half Sleeve T-Shirt', count: 323 },
    ...
    const mainCategories = [
        { name: 'Men', icon: User, color: 'text-white', bg: 'bg-[#1877F2]', border: 'border-transparent', active: true },
        { name: 'Women', icon: User, color: 'text-[#E53E5D]', bg: 'bg-[#FDF2F5]', border: 'border-[#F8D2DB]', active: false },
        { name: 'Kids', icon: Baby, color: 'text-[#10B981]', bg: 'bg-[#ECFDF5]', border: 'border-[#A7F3D0]', active: false },
        { name: 'Teens', icon: CircleUser, color: 'text-[#8B5CF6]', bg: 'bg-[#F5F3FF]', border: 'border-[#DDD6FE]', active: false },
    ];

    // Main Subcategory Pills
    const subcategoryPills = [
        ...
        { name: 'Jacket', count: 30 },
        { name: 'Joggers', count: 18 },
        { name: 'Sweatshirt', count: 15 },
        { name: 'Comfy Trouser', count: 48 },
        { name: 'Sports Trouser', count: 17 },
        { name: 'Shorts', count: 33 },
        { name: 'Underwear', count: 54 },
        { name: 'Socks', count: 36 },
        { name: 'Panjabi', count: 124 },
        { name: 'Tupi', count: 5 },
        { name: 'Jeans', count: 16 },
        { name: 'Pajama', count: 4 },
        { name: 'Accesorries', count: 46 }, // matches screenshot spelling
        { name: 'Chino Pants', count: 12 },
        { name: 'Cargo Pants', count: 18 },
        { name: 'Formal Pants', count: 1 },
    ];

    // Products
    const products = [
        { id: 1, image: pic1, discount: '-24%', name: 'Premium Edition T-Shirt', saveAmt: 'Save ৳210' },
        { id: 2, image: polo1, discount: '-24%', name: 'Premium T-Shirt', saveAmt: 'Save ৳100' },
        { id: 3, image: polo2, discount: '-24%', name: 'Premium T-Shirt', saveAmt: 'Save ৳100' },
        { id: 4, image: polo3, discount: '-22%', name: 'Premium T-Shirt', saveAmt: 'Save ৳100' },
    ];
    ...
    return (
        <div className="flex font-assistant min-h-screen bg-white relative max-w-[1400px] mx-auto border-t border-gray-100 mt-0 pt-0">
            ...
            <aside className="hidden lg:block w-[260px] border-r border-dashed border-gray-300 shrink-0 h-[calc(100vh-80px)] overflow-y-auto sticky top-20 py-4 pr-2">
                ...
                <div className="mb-8 px-4 border-b border-gray-200 pb-6">
                    ...
                    <ul className="space-y-1">
                        {specialOffers.map((offer, idx) => (
                            <li key={idx}>
                                <button
                                    onClick={() => handleOfferClick(idx)} // এখানে ফাংশনটি কল করো
                                    className={`flex items-center w-full text-left px-3 py-2.5 rounded-md transition-colors text-[14px] font-bold
                    ${offer.active ? 'bg-red-50 text-red-600' : 'text-[#4A4B57] hover:bg-red-50'}
                  `}
                                >
                ...
                <div className="px-4">
                    ...
                    <div className="space-y-4">
                        {/* Active Mens Category Menu */}
                        <div>
                            <button className="flex items-center justify-between w-full bg-[#EEF2FF] text-[#1877F2] px-4 py-2.5 rounded-md mb-2">
                                <span className="font-bold text-[14px]">Mens</span>
                                <div className="flex items-center gap-2">
                                    <span className="bg-[#4F46E5] text-white text-[11px] font-bold px-2 py-0.5 rounded-full">1178</span>
                ...
                <div className="p-4 lg:p-6 lg:pl-6">
                    ...
                    <div className="flex flex-wrap gap-x-2.5 gap-y-3.5 mb-8 w-fit max-w-[90%]">
                        {subcategoryPills.map((sub, idx) => (
                            <button
                                key={idx}
                                className="flex items-center gap-1.5 px-4 py-1.5 bg-white border border-gray-200 rounded-full hover:border-[#1877F2] hover:shadow-sm transition-all group"
                            >
                                <span className="text-[14px] font-semibold text-gray-500 group-hover:text-[#1877F2]">{sub.name}</span>
                    ...
                    <div className="flex items-center gap-3 mb-6">
                        <div className="flex items-center bg-gray-100 rounded-md px-3 py-1.5 border border-gray-200">
                            <span className="text-[14px] font-semibold text-[#4A4B57] mr-3">Mens</span>
            ...
            <div className="fixed hidden lg:block right-4 bottom-4 lg:right-6 lg:bottom-6 z-50">
