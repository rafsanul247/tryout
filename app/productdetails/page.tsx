...
import { 
  ...
  ChevronRight, MessageCircle, Zap, Star, Flame, Truck, User, Grid2X2 ,X
} from 'lucide-react';
...
export default function ProductPage() {
  ...
  return (
    <div className="bg-white min-h-screen pb-20 relative">
      ...
      {/* --- উইশলিস্ট মডাল (ওপর থেকে স্লাইড হবে) --- */}
      {isWishlistOpen && (
          ...
          <div className="bg-white w-full max-w-md rounded-lg shadow-2xl overflow-hidden animate-in slide-in-from-top duration-300">
            ...
            <div className="p-8 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-500">Create a wishlist to save items you love and access them anytime.</p>
              </div>

              <Link href="/login"> <button className="cursor-pointer w-full bg-[#1e293b] text-white py-3 rounded-md font-bold hover:bg-slate-800 transition-colors">
                Login Now
              </button></Link>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="space-y-4">
          <div className="bg-[#F3F3F3] rounded-sm relative overflow-hidden group flex max-w-md ml-auto mr-auto lg:ml-20 lg:mr-0">
            <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
              <span className="bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-sm w-fit uppercase">Sale</span>
              <span className="bg-orange-400 text-white text-[10px] font-bold px-2 py-0.5 rounded-sm w-fit uppercase">Free Delivery</span>
            </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-start">
            <h1 className="text-2xl font-bold text-gray-900 leading-tight">Fabrilife Mens Premium T-Shirt- Dhaka</h1>
            <button 
              onClick={() => setIsWishlistOpen(true)}
              className="p-2 border rounded-full hover:bg-red-50 hover:border-red-200 transition-all group"
            >
          ...
          <div className="space-y-1">
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-black text-gray-900">৳ 649</span>
              <span className="text-gray-400 line-through text-sm font-medium">৳ 850.00</span>
              <span className="text-pink-500 text-xs font-bold bg-pink-50 px-2 rounded-full">24% Off</span>
            </div>
            ...
            <div className="flex gap-3">
              {['M', 'L', 'XL', 'XXL'].map((size) => (
                <button
                  ...
                  className={`px-4 py-1.5 border text-xs font-bold transition-all ${selectedSize === size ? 'bg-black text-white border-black' : 'border-gray-200 text-gray-800'}`}
                >
                  {size}
                </button>
              ))}
          ...
          <div className="flex items-center gap-4 pt-2">
            <div className="flex items-center border h-11">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-3 hover:bg-gray-100"><Minus size={16}/></button>
              <span className="w-12 text-center font-bold text-sm">{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className="px-3 hover:bg-gray-100"><Plus size={16}/></button>
            </div>
            <button className="bg-black text-white h-11 px-8 flex-1 flex items-center justify-center gap-2 font-bold hover:bg-gray-800 transition-all uppercase text-sm">
      ...
      {/* Mobile Sticky CTA */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t p-3 flex gap-3 z-50">
        <button className="border p-3 rounded-sm"><MessageCircle size={20}/></button>
        <button className="bg-black text-white flex-1 font-bold text-sm uppercase">Add to Cart</button>
      </div>
      <Footer />
    </div>
  );
